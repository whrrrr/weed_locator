...
class LeRobotIKKeyboardJog(Node):
    ...
    def verify_executed_pose(self, q_start, q_target, start_pose, target_pose):
        ...
        self.get_logger().info(
            'target joint delta deg [pan,lift,elbow,wrist,roll]: '
            f'{np.round(q_target - q_start, 2).tolist()}'
        )
        self.get_logger().info(
            'actual joint delta deg [pan,lift,elbow,wrist,roll]: '
            f'{np.round(q_actual - q_start, 2).tolist()}'
        )
        self.get_logger().info(
            'remaining joint error deg [pan,lift,elbow,wrist,roll]: '
    ...
    def send_command(self, q_target):
        ...
        request.target_positions = q_target.tolist() + [self.gripper_percent]
